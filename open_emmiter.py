...
class ExamplePublisher(object):
    ...
    PUBLISH_INTERVAL = 1
    QUEUE = 'text'
    ROUTING_KEY = 'pi-point'

    def __init__(self, amqp_url, ins):
    ...
    def publish_message(self):
        ...
        properties = pika.BasicProperties(
            ...
            headers=hdrs)
        #with open("hello.json", "r") as read_file:
        #       data = json.load(read_file)
        #message = json.load("load.json","r")
        print (json.dumps(self._in))
        self._channel.basic_publish(self.EXCHANGE, self.ROUTING_KEY,
                                    json.dumps(self._in, ensure_ascii=False),
                                    properties)
        self._message_number += 1
...
def main():
    ...
    example = ExamplePublisher(
        'amqp://guest:guest@localhost:5672/%2F?connection_attempts=3&heartbeat=3600', ins
    )
    example.run()
